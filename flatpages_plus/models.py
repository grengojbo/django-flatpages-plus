...
class PublishedFlatpagesManager(models.Manager):
    def get_query_set(self):
        ...
        return super(PublishedFlatpagesManager, self).get_query_set().exclude(date_unpublish__lte=now).exclude(
            date_publish__gte=now)
...
class Categories(models.Model):
    ...
    status = models.CharField(_('status'), max_length=1, choices=STATUS_LEVELS, default='p',
                              help_text=_(u'Whether or not the page is visible on the site'))
...
class FlatPage(models.Model):
    ...
    url = models.CharField(_('URL'), max_length=150, db_index=True, unique=True)
    title = models.CharField(_('page title'), max_length=200,
                             help_text=_(u'The title of the page is used in the HTML title of the page.'))
    ...
    views = models.IntegerField(_('views'), default=0, blank=True, null=True,
                                help_text=_(
                                    u'The number of the times the page has been viewed (other than the owner).'))
    status = models.CharField(_('status'), max_length=1, choices=STATUS_LEVELS, default='p',
                              help_text=_(u'Whether or not the page is visible on the site'))
    tags = TaggableManager(blank=True,
                           help_text=_(u'A comma seperated list of tags that help to relate pages to each other.'))
    enable_comments = models.BooleanField(_('enable comments'), default=True)
    enable_social = models.BooleanField(_('enable comments'), default=True)
    template_name = models.CharField(_('template name'), max_length=70, blank=True,
                                     help_text=_(
                                         u"Example: 'flatpages_plus/contact_page.html'. If this isn't provided, the system will use 'flatpages/default.html'."))
    ...
    photo = ImageField(_(u'Home Photo'), upload_to=IMAGES_DIR, max_length=255, blank=True, null=True,
                       help_text=_(u"Фото на Главной странице"))
    photo2 = ImageField(_(u'Photo'), upload_to=IMAGES_DIR, max_length=255, blank=True, null=True,
                        help_text=_(u"Фото тексте новости"))
    ...
    description = models.CharField(_(u'Description'), max_length=255, blank=True, null=True,
                                   help_text=_(u"Краткое описание выводится на главной странице"))
    order = models.IntegerField(_('Order'), default=0, help_text=_(u"Сортировать вывод новостей"))
    external_url = models.CharField(_('External URL'), max_length=150, blank=True, null=True,
                                    help_text=_(
                                        u"Ссылка на другую страницу (/catalog/marlin/) сайта или другой сайт (http://sky5.com.ua/)"))
    gonews = models.BooleanField(_(u'Перейтик к подробнее'), default=True,
                                 help_text=_(u"Если активно то при клике будет переход на страницу подробне"))
    ...
    def photo_thumbnail(self):
        try:
            ...
            return '<img src="{url}" width="{width}" height="{height}"">'.format(url=im.url, width=im.width,
                                                                                 height=im.height)
        except IOError:
